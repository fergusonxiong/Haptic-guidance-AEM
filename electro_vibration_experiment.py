...
fontTargets = pygame.font.Font('freesansbold.ttf', 70) # printing text font and font size
textTargets = fontTargets.render('Targets (' + str(0) + '/' + str(0) +')', True, (0, 0, 0)) # printing text object
textTargetsRect = textTargets.get_rect()
textTargetsRect.center = (xc, yc)

fontTargets1 = pygame.font.Font('freesansbold.ttf', 50) # printing text font and font size
textTargets1 = fontTargets1.render('Targets:', True, (0, 0, 0)) # printing text object
textTargets1Rect = textTargets1.get_rect()
textTargets1Rect.center = (xc, yc-100)

fontTargets2 = pygame.font.Font('freesansbold.ttf', 100) # printing text font and font size
textTargets2 = fontTargets2.render('(' + str(0) + '/' + str(10) +')', True, (0, 0, 0)) # printing text object
textTargets2Rect = textTargets2.get_rect()
textTargets2Rect.center = (xc, yc+20)
...
sinewave = SineWave(pitch = 0, pitch_per_second = 40)
...
while run:
    ...
    if np.sqrt((xh[0]-target_pos[0])**2 + (xh[1]-target_pos[1])**2)<target_radius:
        target_pos = np.array([random.randint(x_plot[0], x_plot[1]), random.randint(y_plot[0], y_plot[1])])  # random target pos
        target_n = target_n+1
        window.fill((0,255,0)) # clear window
        pygame.display.flip() # update display
        time.sleep(0.5)
    ...
    textTargets2 = fontTargets2.render('(' + str(target_n) + '/' + str(target_n_total) +')', True, (255, 0, 0)) # printing text object
    window.blit(textTargets1, textTargets1Rect)
    window.blit(textTargets2, textTargets2Rect)
